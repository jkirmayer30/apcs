...
public class LList<T> implements List<T> //Q: Why no "implements Iterable"
    //A: Iterable is implemented by virtue of implementing List
{
    // Your List.java must be in same dir to supersede
    // built-in Java List interface

    //instance vars/* YOUR CODE HERE */
    ...
    private class MyIterator implements Iterator<T> {
        ...
        public void remove() {
            if (_okToRemove) {
                if (_dummy.getPrev() != null && _dummy.getPrev().getPrev() != null && !atEnd) {
                    _dummy.getPrev().getPrev().setNext(_dummy);
                    _dummy.setPrev(_dummy.getPrev().getPrev());
                }

                if (atEnd && _dummy.getPrev() != null) {
